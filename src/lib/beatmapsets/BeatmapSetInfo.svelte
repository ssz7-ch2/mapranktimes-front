<script lang="ts">
	import ModeIcon from '$lib/ModeIcon.svelte';
...
{#if moreInfo}
	<span
		class="absolute bottom-0 left-0 w-full h-3 bg-[#353535] -z-20"
		style={unresolved ? 'filter: brightness(0.5)' : undefined}
		in:fade={{ duration: 200 }}
		out:fade={{ duration: 150 }}
	></span>
	<div
		class="absolute top-28 md:top-36 w-full z-30 pointer-course:pointer-events-none"
		style={unresolved ? 'filter: brightness(0.5)' : undefined}
	...
	>
		<div
			class="bg-[#353535] rounded-b-[12px] p-2 text-xs font-light text-left grid grid-cols-[max-content_max-content_max-content_auto] gap-y-0.5 gap-x-2"
		>
			{#each beatmaps as beatmap}
				...
				<div class="flex flex-row gap-1 items-center overflow-hidden">
					{#if $selectedMode === -1}
						...
						/>
					{/if}
					<p class="font-normal truncate">{beatmap.ver}</p>
				</div>
			{/each}
		</div>
		<div
			class="absolute top-0 flex flex-col h-full w-full pointer-events-auto rounded-b-[12px] p-2 justify-between"
		>
			{#each beatmaps as beatmap}
